    ...
    async.parallel([
        ...
        function (callback) {
            async.map([ "base.dat", "check.dat" ], function (filename, _callback) {
                loadArrayBuffer(new URL(filename, dic_path).toString(), function (err, buffer) {
                    if(err) {
        ...
        function (callback) {
            loadArrayBuffer(new URL("cc.dat", dic_path).toString(), function (err, buffer) {
                if(err) {
                    return callback(err);
                }
...
},{"../dict/DynamicDictionaries":8,"async":1}],18:[function(require,module,exports){
/*
 * Copyright 2014 Takuya Asano
 * Copyright 2010-2014 Atilika Inc. and contributors
 *
